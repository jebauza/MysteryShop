...
@stop
 ...
 <li><a href="{{URL::route('index')}}">Home</a></li>
         <li><a href="{{URL::route('market_list')}}">Store {{$departament->market->num_market}}</a></li>
         <li><a href="{{URL::route('departament_list_market',[$departament->market->id_market])}}">Departments {{$departament->name}}</a></li>
...
@stop
 @section('body_tabla')
 <?php
 $indicators_dept = DB::table('tb_indicator_departament')
                 ->join('tb_indicator','tb_indicator_departament.id_indicator','=','tb_indicator.id_indicator')
                 ->where('tb_indicator_departament.id_departament','=',$departament->id_dpto)
     ...
     @foreach($indicators_dept as $indic)
                   ...
                   <td data-original-title="{{$indic->description}}" data-toggle="tooltip" data-placement="top" title="" >{{$indic->name}}
                     ...
                     <div id="" class="pull-right hidden visible-xs btn-flotantes">
                        <span><a href="{{URL::route("indicator_edit",[$indic->id_indicator,'dep',$departament->id_dpto])}}" class="col-xs-6 "><i class="fa fa-edit "></i></a> </span>
                        <span><a href="#deleteModal" data-toggle="modal" data-url="{{URL::route('indicator_delete',[$indic->id_indicator,'departament',$departament->id_dpto])}}" class="col-xs-6 btn-delete"><i class=" fa fa-trash-o "></i></a> </span>
                      </div>
                   </td>
...
@section('modal')
       @parent
          ...
          aria-labelledby="insertModal" aria-hidden="true">
                              ...
                              {{ Form::open(array('url' => URL::route("indicator_departament_add"),'method'=>"post",'id'=>"form-indi",'class'=>"form-horizontal")) }}
                                           ...
                                           <div class="radio">
                                               ...
                                               <div class="form-group exist hidden">
                                                   ...
                                                   <div class="col-sm-12">
                                                       ...
                                                       @foreach($arrIndicadoresResultado as $indic)
                                                           <option value="{{$indic['id_indicator']}}" title="{{$indic['description']}}">{{$indic['name']}}</option>
                                                       @endforeach
                                                       </select>
                                                   </div>
                                               </div>
